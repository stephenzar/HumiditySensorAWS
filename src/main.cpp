...
extern const uint8_t aws_root_ca_pem_start[] asm("_binary_
AmazonRootCA1_pem_start");
extern const uint8_t aws_root_ca_pem_end[] asm("_binary_
AmazonRootCA1_pem_end");
extern const uint8_t certificate_pem_crt_start[] asm("_binary_
my_sensor1_cert_pem_start");
extern const uint8_t certificate_pem_crt_end[] asm("_binary_my_
sensor1_cert_pem_end");
extern const uint8_t private_pem_key_start[] asm("_binary_my_
sensor1_private_key_start");
extern const uint8_t private_pem_key_end[] asm("_binary_my_
sensor1_private_key_end");
...
{
xTaskCreate(connect_aws_mqtt, "connect_aws_mqtt", 15 * 
...
iotClientInitParamsDefault;
  ...
  mqttInitParams.pRootCALocation = (const char *)aws_root_ca_
pem_start;
  mqttInitParams.pDeviceCertLocation = (const char *)
certificate_pem_crt_start;
  mqttInitParams.pDevicePrivateKeyLocation = (const char *)
...
iotClientConnectParamsDefault;
  ...
  while (aws_iot_mqtt_connect(&aws_client, &connectParams) != 
SUCCESS)
  {
...
aws_iot_mqtt_subscribe(&aws_client, ENABLE_TOPIC, 
strlen(ENABLE_TOPIC), QOS0, subscribe_handler, NULL);
  while (1)
  {
...
static void publish_reading(int temp, int hum)
{
  ...
  aws_iot_mqtt_publish(&aws_client, TEMP_TOPIC, strlen(TEMP_
TOPIC), &message);
  itoa(hum, buffer, 10);
  message.payloadLen = strlen(buffer);
  aws_iot_mqtt_publish(&aws_client, HUM_TOPIC, strlen(HUM_
TOPIC), &message);
}
